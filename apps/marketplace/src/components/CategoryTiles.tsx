...
/** "Shop by category": a large tile per category that has active comparisons, plus one for everything. */
    ...
    <section className={`container ${styles.section}`} aria-labelledby="shop-by-category">
      ...
      <ul className={styles.list}>
        {groups.map((group) => {
          ...
          return (
            <li key={group.slug}>
              <Link to={`/category/${group.slug}`} className={styles.tile}>
        ...
        <li>
          <Link to="/search" className={`${styles.tile} ${styles.all}`}>
            <span className={styles.iconWrap}>
            ...
            <span className={styles.text}>
              ...
              <span className="caption">
                {total} {total === 1 ? 'comparison' : 'comparisons'}, biggest savings first
              </span>
            </span>
            <ArrowRightIcon className={styles.arrow} />
          </Link>
